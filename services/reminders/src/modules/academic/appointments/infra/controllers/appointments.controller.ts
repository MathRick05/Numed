...
export class AppointmentsController {
  ...
  @HateoasList<AppointmentResponseDto>({
    basePath: "/v1/users/:userId/appointments",
    itemLinks: (item) => ({
      self: { href: `/v1/users/${item.userId}/appointments/${item.id}`, method: "GET" },
      update: { href: `/v1/users/${item.userId}/appointments/${item.id}`, method: "PUT" },
      delete: { href: `/v1/users/${item.userId}/appointments/${item.id}`, method: "DELETE" },
    }),
  })
  ...
  @HateoasItem<AppointmentResponseDto>({
    basePath: "/v1/users/:userId/appointments",
    itemLinks: (item) => ({
      self: { href: `/v1/users/${item.userId}/appointments/${item.id}`, method: "GET" },
      update: { href: `/v1/users/${item.userId}/appointments/${item.id}`, method: "PUT" },
      delete: { href: `/v1/users/${item.userId}/appointments/${item.id}`, method: "DELETE" },
      list: { href: `/v1/users/${item.userId}/appointments`, method: "GET" },
    }),
  })
  async findById(
    @Param("userId") userId: string,
