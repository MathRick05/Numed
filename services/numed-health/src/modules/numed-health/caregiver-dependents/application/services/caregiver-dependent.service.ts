...
} from "@numed-health/caregiver-dependents/domain/repositories/caregiver-dependent-repository.interface";
...
export class CaregiverDependentService {
  ...
  async addDependent(caregiverId: string, dependentId: string): Promise<void> {
    const exists = await this.caregiverDependentRepository.exists(caregiverId, dependentId);
