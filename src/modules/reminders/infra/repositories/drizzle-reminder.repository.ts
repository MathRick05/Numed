...
import type { ReminderRepository } from "@reminders/domain/repositories/reminder-repository.interface";
...
export class DrizzleReminderRepository implements ReminderRepository {
  ...
  async findAllByUserId(userId: string): Promise<Reminder[]> {
    ...
    return rows.map((row) => Reminder.restore(row)!);
  }
  ...
  async findAllByUserIdPaginated(
    ...
    return { rows: rows.map((row) => Reminder.restore(row)!), total: countResult.count };
  }
}
